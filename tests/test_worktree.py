...
class TestRemoveWorktreePathSelection:
    ...
    def test_fallback_to_git_query_when_no_path_provided(self, tmp_path):
        ...
        with patch.object(wm, "list_worktrees") as mock_list, \
...
class TestCancelTaskWorktreeCleanup:
    ...
    def test_cancel_clears_fields_on_successful_worktree_removal(self, tmp_db, make_task):
        task = make_task(
    ...
    def test_cancel_preserves_fields_on_failed_worktree_removal(self, tmp_db, make_task):
        task = make_task(
...
class TestCascadingCancel:
    def test_cancel_cascades_to_children(self, tmp_db, make_task):
        ...
        c1 = make_task(status=TaskStatus.CODING, parent_id=parent.id,
                       branch_name="agent/c1", worktree_path="/wt/c1")
        c2 = make_task(status=TaskStatus.PLANNING, parent_id=parent.id)
        tmp_db.save_task(c1); tmp_db.save_task(c2)
        orch = _orch_helper(tmp_db)
    ...
    def test_cancel_cascades_recursively(self, tmp_db, make_task):
        gp = make_task(status=TaskStatus.PLANNING); tmp_db.save_task(gp)
        p = make_task(status=TaskStatus.CODING, parent_id=gp.id,
                      branch_name="agent/p", worktree_path="/wt/p")
        tmp_db.save_task(p)
        c = make_task(status=TaskStatus.CODING, parent_id=p.id,
                      branch_name="agent/c", worktree_path="/wt/c")
        tmp_db.save_task(c)
        orch = _orch_helper(tmp_db)
    ...
    def test_cancel_skips_completed_children(self, tmp_db, make_task):
        parent = make_task(status=TaskStatus.PLANNING); tmp_db.save_task(parent)
        done = make_task(status=TaskStatus.COMPLETED, parent_id=parent.id)
        active = make_task(status=TaskStatus.CODING, parent_id=parent.id,
                           branch_name="a/x", worktree_path="/wt/x")
        tmp_db.save_task(done); tmp_db.save_task(active)
        orch = _orch_helper(tmp_db)
        orch.cancel_task(parent.id)
...
class TestCascadingClean:
    def test_clean_cascades_to_children(self, tmp_db, make_task):
        parent = make_task(status=TaskStatus.COMPLETED,
                           branch_name="agent/p", worktree_path="/wt/p")
        tmp_db.save_task(parent)
        c = make_task(status=TaskStatus.COMPLETED, parent_id=parent.id,
                      branch_name="agent/c", worktree_path="/wt/c")
        tmp_db.save_task(c)
        orch = _orch_helper(tmp_db)
    ...
    def test_clean_skips_children_without_branch(self, tmp_db, make_task):
        parent = make_task(status=TaskStatus.CANCELLED,
                           branch_name="agent/p", worktree_path="/wt/p")
        tmp_db.save_task(parent)
...
class TestNoRecursiveSplit:
    ...
    def _make_full_orchestrator(self, tmp_db):
        ...
        orch._coder_by_complexity = {}
        orch._default_coder = MagicMock()
        orch._default_coder.implement_task.return_value = (self._make_coder_run(), "done")
        orch.reviewers = []
    ...
    def _make_plan_run(self, task_id=""):
        ...
        return AgentRun(task_id=task_id, agent_type="planner", model="m",
                        prompt="p", output="o", exit_code=0, session_id="")

    def _make_coder_run(self, task_id=""):
        from core.models import AgentRun
        run = AgentRun(task_id=task_id, agent_type="coder", model="m",
                       prompt="p", output="o", exit_code=0, session_id="s1")
        return run
    ...
    def test_planner_subtask_ignores_split_true(self, tmp_db, make_task):
        ...
        orch.planner.analyze_and_split.return_value = (
            ...
            "",
            [{"title": "A", "description": "a", "priority": "medium", "depends_on": []},
             {"title": "B", "description": "b", "priority": "medium", "depends_on": []}],
            "complex",
        )
    ...
    def test_manual_task_can_still_split(self, tmp_db, make_task):
        ...
        orch.planner.analyze_and_split.return_value = (
            ...
            "",
            [{"title": "A", "description": "a", "priority": "medium", "depends_on": []},
             {"title": "B", "description": "b", "priority": "medium", "depends_on": [1]}],
            "complex",
        )
